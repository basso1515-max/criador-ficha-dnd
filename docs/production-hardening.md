...
- Trate `OPENAI_API_KEY` como segredo de produção. Se a chave for exposta, revogue no painel da OpenAI e atualize a Vercel antes de reabilitar o assistente.
- Se trocar `OPENAI_CHARACTER_MODEL`, valide `/api/ai-character` e uma geração real antes de anunciar o recurso.
