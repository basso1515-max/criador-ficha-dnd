...
import {
  ...
  bindDropdownSuggestionInteraction as bindDropdownSuggestionInteraction2024,
  consumeDropdownInteractionBlur as consumeDropdownInteractionBlur2024,
  createCustomSelectController,
} from "../custom-select-ui.js";
import {
...
import {
  ABILITY_LABELS,
  ABILITY_ORDER,
  OMITTED_PDF_FEATURE_NAMES_2024,
  STANDARD_ABILITY_SET_2024,
  STANDARD_ABILITY_SET_BY_CLASS_2024,
  POINT_BUY_COSTS_2024,
  DISTANCE_UNITS_2024,
  WEIGHT_UNITS_2024,
...
import {
  ...
  SUBCLASS_ALIGNMENT_RULES_2024,
  SPELL_LEVEL_LABELS_2024,
  MAGIC_SPELL_TAG_FILTERS_2024,
  MAGIC_FILTER_DEFAULTS_2024,
  SPELL_SLOT_LEVELS_2024,
  SLOT_TABLES_2024,
  PREPARED_FULL_SPELLS_2024,
  PREPARED_SORCERER_SPELLS_2024,
  PREPARED_WIZARD_SPELLS_2024,
  PREPARED_HALF_SPELLS_2024,
  SPELLCASTING_RULES_2024,
  SUBCLASS_SPELLCASTING_RULES_2024,
  MULTICLASS_PREREQUISITES_2024,
  MULTICLASS_PROFICIENCIES_2024,
  MULTICLASS_SPELLCASTER_SLOT_TABLE_2024,
  SIZE_LABELS,
  ARMOR_LABELS,
  WEAPON_LABELS,
  DAMAGE_TYPE_LABELS_2024,
  LANGUAGE_LABELS_2024,
  ...
  PDF_MAP_URL_2024,
  DEFAULT_TEMPLATE_URL_2024,
  FEAT_CATEGORY_LABELS,
  SKILL_OPTIONS,
  SKILL_ABILITY_MAP,
  TOOL_LABELS,
  buildFeatToolProficiencyOptions2024,
  LEGACY_BACKGROUND_ID_2024,
  LEGACY_BACKGROUND_ORIGIN_FEAT_SLOT_ID_2024,
...
import {
  ...
  WIZARD_SUBCLASS_GRANTED_SPELL_IDS_2024,
  PALADIN_CHANNEL_DIVINITY_BY_LEVEL_2024,
  PALADIN_DEVOTION_GRANTED_SPELL_IDS_2024,
  PALADIN_GLORY_GRANTED_SPELL_IDS_2024,
  PALADIN_VENGEANCE_GRANTED_SPELL_IDS_2024,
  PALADIN_ANCIENTS_GRANTED_SPELL_IDS_2024,
...
import {
  ...
  BARD_MAGICAL_SECRETS_CLASS_IDS_2024,
  BARD_WORDS_OF_CREATION_SPELL_IDS_2024,
  BARD_GLAMOUR_GRANTED_SPELL_IDS_2024,
...
(() => {
  ...
  const {
    commitValue: commitCustomSelectValue2024,
    createField: createCustomSelectField2024,
    syncField: syncCustomSelectField2024,
  } = createCustomSelectController({
    fields: CUSTOM_SELECT_FIELDS_2024,
    escapeHtml,
    clearLabel: "Limpar",
  });
  ...
  const el = {
    ...
    userAreaContainer: document.getElementById("userAreaContainer2024"),
    userArea: document.getElementById("userArea2024"),
    userAreaHeader: document.getElementById("userAreaHeader2024"),
    ...
    editorLogout: document.getElementById("editorLogout2024"),
    mobileMenuShell: document.getElementById("mobileEditorMenuShell2024"),
    mobileMenuToggle: document.getElementById("mobileMenuToggle2024"),
    mobileMenu: document.getElementById("mobileEditorMenu2024"),
    mobileCharacterBlock: document.getElementById("mobileCurrentCharacter2024"),
  ...
  function getAdditionalMulticlassLevelValues2024(excludeRow = null) {
    return getAdditionalMulticlassRows2024()
      .filter((row) => !excludeRow || row !== excludeRow)
      .map((row) => row.querySelector("[data-multiclass-level]")?.value);
  }

  function getNormalizedAdditionalMulticlassRows2024(totalLevel = getSelectedLevel()) {
    const rows = getAdditionalMulticlassRows2024();
    const normalized = normalizeMulticlassAdditionalLevels2024(
      totalLevel,
  ...
  function getAssignedAdditionalLevels2024(excludeRow = null) {
    ...
    return normalizeMulticlassAdditionalLevels2024(totalLevel, getAdditionalMulticlassLevelValues2024(excludeRow))
      .reduce((sum, entry) => sum + entry.level, 0);
  }

  function getComputedPrimaryLevel2024() {
    const totalLevel = getSelectedLevel();
    return getPrimaryLevelFromMulticlassDistribution2024(totalLevel, getAdditionalMulticlassLevelValues2024());
  }
  ...
  function collectClassEntries2024(primaryClass = getSelectedClass(), primarySubclass = getSelectedSubclass(), totalLevel = getSelectedLevel()) {
    ...
    getNormalizedAdditionalMulticlassRows2024(totalLevel).forEach(({ row, level }, index) => {
  ...
  function getSpellcastingRuleCount2024(classId, property, level) {
    return Number(SPELLCASTING_RULES_2024[classId]?.[property]?.[level] || 0);
  }

  const CLASS_RESOURCE_SUMMARY_BUILDERS_2024 = {
    barbaro(level) {
    ...
    bardo(level) {
      ...
      const cantrips = getSpellcastingRuleCount2024("bardo", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("bardo", "preparedByLevel", level);
    ...
    clerigo(level) {
      ...
      const cantrips = getSpellcastingRuleCount2024("clerigo", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("clerigo", "preparedByLevel", level);
    ...
    bruxo(level) {
      ...
      const cantrips = getSpellcastingRuleCount2024("bruxo", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("bruxo", "preparedByLevel", level);
    ...
    druida(level) {
      const cantrips = getSpellcastingRuleCount2024("druida", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("druida", "preparedByLevel", level);
    ...
    feiticeiro(level) {
      const cantrips = getSpellcastingRuleCount2024("feiticeiro", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("feiticeiro", "preparedByLevel", level);
    ...
    mago(level) {
      const cantrips = getSpellcastingRuleCount2024("mago", "cantripsByLevel", level);
      const prepared = getSpellcastingRuleCount2024("mago", "preparedByLevel", level);
    ...
    paladino(level) {
      const prepared = getSpellcastingRuleCount2024("paladino", "preparedByLevel", level);
  ...
  function buildClassResourceSummary2024(classId, classLevel) {
    const builder = CLASS_RESOURCE_SUMMARY_BUILDERS_2024[classId];
    return builder ? builder(clampInt(classLevel, 1, 20)) : "";
  }
  ...
  function normalizeAdditionalMulticlassLevels2024() {
    ...
    getNormalizedAdditionalMulticlassRows2024(totalLevel).forEach(({ row, level, max }) => {
      const levelInput = row.querySelector("[data-multiclass-level]");
  ...
  function updateMulticlassSummary2024() {
    ...
    const normalizedByRow = new Map(
      getNormalizedAdditionalMulticlassRows2024(totalLevel).map(({ row, level }) => [row, level])
    );
    let assignedLevels = primaryClass ? getPrimaryAssignedLevel2024() : 0;

    if (!primaryClass) {
      warnings.push("Selecione a classe principal antes de distribuir niveis.");
    }

    if (primaryClass && rows.length) {
      const primaryPrereqValidation = validateMulticlassPrerequisites2024(primaryClass.id);
      if (primaryPrereqValidation.applicable && !primaryPrereqValidation.valid) {
        warnings.push(
          primaryPrereqValidation.resolved
            ? `A classe principal ${primaryClass.nome} tambem exige ${getMulticlassRequirementText2024(primaryClass.id)} para multiclasse.`
            : `Complete os atributos finais para validar a classe principal ${primaryClass.nome}.`
        );
      }
    }
    ...
    rows.forEach((row, index) => {
      const classId = String(row.querySelector("[data-multiclass-class]")?.value || "").trim();
      assignedLevels += normalizedByRow.get(row) || clampInt(row.querySelector("[data-multiclass-level]")?.value, 1, totalLevel);
      if (!CLASS_BY_ID.has(classId)) {
        warnings.push(`A multiclasse ${index + 1} ainda nao tem uma classe selecionada.`);
      }
    });
  ...
  function normalizeAbilityScoreMap2024(rawScores = {}) {
    return Object.fromEntries(ABILITY_ORDER.map((ability) => {
      ...
      return [ability, Number.isFinite(value) ? value : null];
    }));
  }

  function readBaseAbilityScoreControlValues2024(rootElement = el.abilityScores) {
    return Object.fromEntries(ABILITY_ORDER.map((ability) => {
      const field = rootElement?.querySelector(`[name="base-${ability}"]`);
      return [ability, field?.value || ""];
    }));
  }

  function getBaseAbilityScores() {
    return normalizeAbilityScoreMap2024(readBaseAbilityScoreControlValues2024());
  }
  ...
  function collectPreviewState2024() {
    syncAlignmentInfo2024();
    syncDivinityInfo2024();
  ...
  function buildSummaryPreviewItems2024({
    alignmentLabel,
    background,
    classDistribution,
    classEntries,
  ...
  function buildOriginPreviewItems2024({
    abilitySummary,
    background,
    effectiveAbilityScores,
    featSummary,
  ...
  function buildCombatPreviewItems2024({
    derivedCombat,
    initiativeBonus,
    perceptionBonus,
    proficiencyBonus,
  ...
  function buildClassEntryPreviewItem2024(entry) {
    const classFeatures = formatList(collectUnlockedFeatureNames(entry.classData?.features, entry.level));
    const subclassFeatures = entry.subclassData
      ? formatList(collectUnlockedFeatureNames(entry.subclassData?.features, entry.level))
      : "";
    ...
    return previewItem(
      ...
      [
        ...
        classFeatures ? `Recursos: ${classFeatures}` : "",
        subclassFeatures ? `Recursos da subclasse: ${subclassFeatures}` : "",
      ].filter(Boolean).join(" • ")
    );
  }
  ...
  function buildSpeciesPreviewItems2024({ race, subrace }) {
    return race
      ? [
  ...
  function buildEquipmentPreviewItems2024({
    currencyBreakdown,
    equipmentSummary,
    shoppingState,
    weaponRows,
  }) {
    return equipmentSummary.length
      ? [
  ...
  function buildSpellPreviewItems2024({
    selectedSpellEntries,
    slotPool,
    spellContext,
    spellPageData,
  }) {
    return spellContext?.sources?.length
      ? [
  ...
  function buildNotePreviewItems2024() {
    return [
  ...
  function buildContextualCrestMarkup2024(previewState) {
    ...
    const iconPath = divinity || !previewState.cls ? "" : `/assets/icons/classes/2024/${previewState.cls.id}.svg`;
    const selectedDivinity = divinity
      ? DIVINITY_BY_NAME_2024.get(normalizePt(divinity))
      : null;
    const sourceLabel = divinity || previewState.cls?.nome || "Build sem símbolo";
    ...
    const sourceDescription = selectedDivinity?.descricaoCurta
      ...
      || (previewState.cls ? "Representa a identidade mecânica da classe atual." : "Escolha uma classe ou divindade para definir o brasão.");
  ...
  function updatePreview() {
    ...
    el.preview.innerHTML = buildPreviewHtml2024(collectPreviewState2024());
  }
  ...
  function getSpeciesChoiceDefinitions(race, subrace) {
    ...
    const choices = [];
    const hiddenTraitChoiceIds = new Set();

    if (race.id === "golias" && Array.isArray(race?.subracas) && race.subracas.length) {
      hiddenTraitChoiceIds.add("ancestralidade-gigante");
    }
    ...
    [...(race?.tracos || []), ...(subrace?.tracos || [])].forEach((trait) => {
      ...
      if (hiddenTraitChoiceIds.has(trait.id)) return;
      choices.push({
